...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // if everything is ok, try to upload file
        ...
        if (move_uploaded_file($_FILES["fileToUpload"]["tmp_name"], $target_file)) {
            ...
            $sql = "INSERT INTO uploaded_documents (filename, file_path) VALUES ('".basename($_FILES["fileToUpload"]["name"])."', '$target_file')";

            if ($conn->query($sql) === TRUE) {
                echo "File data saved successfully";
            } else {
                echo "Error saving file data: " . $conn->error;
            }
        } else {
